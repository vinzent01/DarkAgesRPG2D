...
public class InventoryItemWidget : Widget {
    ...
    {
        var actionMenu = new ActionMenu(State.player, Item);
...
public class InventoryWidget : Widget{
    ...
    protected override bool OnDropOnGround(Widget widget) {
        ...
        if (widget is InventoryItemWidget itemWidget) {
            ...
            // todo : fix hard coded inventory remove
            // because action system does not alows objects
            // to take drop action
            var ownerInventory = owner.GetComponent<Inventory>();

            if (ownerInventory != null){
                ownerInventory.RemoveItem(item);
                State.player.AppendAction(dropAction, item);
            }
    ...
    protected override void OnClose(){
        ...
        if (owner.HasComponentType<ContainerComponent>()){
            ...
            if (CloseAction.MeetsCondition(State.player, owner)){
                CloseAction.StartAction(State.player, owner);
            }
        }
    }
}